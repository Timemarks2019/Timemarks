...
import com.asoft.timemarks.Utils.AppSignatureHashHelper;
...
public class OtpVerification extends BaseActivity implements SMSReceiver.OTPReceiveListener {
    TextView tvText;
    TextView txtMessage;
    ...
    private void startTimer() {
        ...
        tvResendTimer.setTextColor(ContextCompat.getColor(OtpVerification.this, R.color.grayfm));
        new CountDownTimer(30000, 1000) {
            ...
            public void onFinish() {
                ...
                tvResendTimer.setTextColor(ContextCompat.getColor(OtpVerification.this, R.color.colorPrimary));
            }
        }.start();
    }
    ...
    private void reSendOtp(String mobile) {
        ...
        Call<ResSendOtp> call = apiService.reSendOtp(mobile);
        call.enqueue(new Callback<ResSendOtp>() {
            ...
            public void onResponse(Call<ResSendOtp> call, Response<ResSendOtp> response) {
                ...
                if (response.isSuccessful()) {
                    ...
                    Toast.makeText(OtpVerification.this, response.message(), Toast.LENGTH_LONG).show();
                }
            }
            ...
            public void onFailure(Call<ResSendOtp> call, Throwable t) {
                hideLoading();
                Toast.makeText(OtpVerification.this, "NetworkCallFailure : "+t, Toast.LENGTH_LONG).show();
            }
        });
    }
    @Override
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQ_CODE_FINISH) {
            if (resultCode == RESULT_OK) {
                if (smsReceiver != null) {
                    LocalBroadcastManager.getInstance(OtpVerification.this).unregisterReceiver(smsReceiver);
                }
    ...
    private void checkOtp(final String mobile, final String otp) {
        showLoading();
        ApiInterface apiService = ApiClient.getClient().create(ApiInterface.class);
        Call<ResSendOtp> call = apiService.checkOtp(mobile,otp);
        call.enqueue(new Callback<ResSendOtp>() {
            @Override
            public void onResponse(Call<ResSendOtp> call, Response<ResSendOtp> response) {
                hideLoading();
                if (response.isSuccessful()) {
                    if (response.body().getStatus()) {
                        Intent intent = new Intent(OtpVerification.this, ChangePasswordWithOtp.class);
